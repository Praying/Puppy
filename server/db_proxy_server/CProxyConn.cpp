 ...
 */
    ...
    {
        if (sig_no == SIGTERM) {
            LOG(ERROR)<<"receive SIGTERM, prepare for exit";
            CImPdu cPdu;
...
//////////////////////////
    ...
    void CProxyConn::OnConnect(net_handle_t handle)
    {
        ...
        g_proxy_conn_map.insert(std::make_pair(handle, this));
        ...
        LOG(ERROR)<<"connect from "<<m_peer_port<<", handle="<< m_handle;
    }
...
// 由于数据包是在另一个线程处理的，所以不能在主线程delete数据包，所以需要Override这个方法
    void CProxyConn::OnRead()
    {
        ...
        try {
            ...
            LOG(ERROR)<<"!!!catch exception, err_code="<<ex.GetErrorCode()<<", err_msg="<<ex.GetErrorMsg()<<", close the connection ";
            OnClose();
        }
    ...
    void CProxyConn::OnTimer(uint64_t curr_tick)
    {
        ...
        if (curr_tick > m_last_recv_tick + SERVER_TIMEOUT) {
            LOG(ERROR)<<"proxy connection timeout "<< m_peer_ip<<":"<< m_peer_port;
            Close();
        }
    }
    ...
    void CProxyConn::HandlePduBuf(uchar_t* pdu_buf, uint32_t pdu_len)
    {
        ...
        if (handler) {
            ...
            LOG(ERROR)<<"no handler for packet type: " <<pPdu->GetCommandId();
        }
    }
