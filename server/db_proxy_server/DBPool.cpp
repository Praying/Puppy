...
namespace Flow {
    ...
    CResultSet::CResultSet(MYSQL_RES *res) {
        ...
        // std::map table field key to index in the result array
...
/////////////////////////////////////////
    ...
    void CPrepareStatement::SetParam(uint32_t index, uint32_t &value) {
        if (index >= m_param_cnt) {
            LOG(ERROR) << "index too large: %d" << index;
            return;
        }
...
////////////////
    ...
    int CDBPool::Init() {
        ...
        LOG(ERROR) << "db pool: " << m_pool_name << ", size: " << (int) m_free_list.size();
        return 0;
    }
        ...
        while (m_free_list.empty()) {
            if (m_db_cur_conn_cnt >= m_db_max_conn_cnt) {
                ...
                if (ret) {
                    ...
                    m_free_list.push_back(pDBConn);
                    m_db_cur_conn_cnt++;
                    LOG(ERROR) << "new db connection: " << m_pool_name << ", conn_cnt: %d" << m_db_cur_conn_cnt;
                }
            }
        }
 ...
 */
    int CDBManager::Init() {
        ...
        if (!db_instances) {
            LOG(ERROR) << "not configure DBInstances";
            return 1;
        }
        ...
        char password[64];
        char maxconncnt[64];
        Util::CStrExplode instances_name(db_instances, ',');
        ...
        for (uint32_t i = 0; i < instances_name.GetItemCnt(); i++) {
            ...
            if (!db_host || !str_db_port || !db_dbname || !db_username || !db_password || !str_maxconncnt) {
                LOG(ERROR) << "not configure db instance:" << pool_name;
                return 2;
            }

            int db_port = atoi(str_db_port);
            int db_maxconncnt = atoi(str_maxconncnt);
            CDBPool *pDBPool = new CDBPool(pool_name, db_host, db_port, db_username, db_password, db_dbname,
                                           db_maxconncnt);
            if (pDBPool->Init()) {
                LOG(ERROR) << "init db instance failed: %s" << pool_name;
                return 3;
            }
