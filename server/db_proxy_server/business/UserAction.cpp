...
namespace Flow::DB_PROXY {
    void getUserInfo(CImPdu *pPdu, uint32_t conn_uuid) {
        ...
        if (msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength())) {
            ...
            RAW_LOG(INFO, "userId=%u, userCnt=%u", from_user_id, userCount);
    ...
    void getChangedUser(CImPdu *pPdu, uint32_t conn_uuid) {
        ...
        if (msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength())) {
            ...
            RAW_LOG(INFO, "userId=%u,nLastUpdate=%u, last_time=%u, userCnt=%u", nReqId, nLastUpdate, nLastTime,
    ...
    void changeUserSignInfo(CImPdu *pPdu, uint32_t conn_uuid) {
        ...
        if (req.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength())) {
            ...
            if (result) {
                ...
                RAW_LOG(INFO, "changeUserSignInfo sucess, user_id=%u, sign_info=%s", user_id, sign_info.c_str());
            } else {
                RAW_LOG(INFO, "changeUserSignInfo false, user_id=%u, sign_info=%s", user_id, sign_info.c_str());
            }
            ...
            RAW_LOG(INFO, "changeUserSignInfo: IMChangeSignInfoReq ParseFromArray failed!!!");
        }
    }
    ...
    void doPushShield(CImPdu *pPdu, uint32_t conn_uuid) {
        ...
        if (req.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength())) {
            ...
            if (result) {
                ...
                RAW_LOG(INFO, "doPushShield sucess, user_id=%u, shield_status=%u", user_id, shield_status);
            } else {
                RAW_LOG(INFO, "doPushShield false, user_id=%u, shield_status=%u", user_id, shield_status);
            }
            ...
            RAW_LOG(INFO, "doPushShield: IMPushShieldReq ParseFromArray failed!!!");
        }
    }
    ...
    void doQueryPushShield(CImPdu *pPdu, uint32_t conn_uuid) {
        ...
        if (req.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength())) {
            ...
            if (result) {
                resp.set_shield_status(shield_status);
                RAW_LOG(INFO, "doQueryPushShield sucess, user_id=%u, shield_status=%u", user_id, shield_status);
            } else {
                RAW_LOG(INFO, "doQueryPushShield false, user_id=%u", user_id);
            }
            ...
            RAW_LOG(INFO, "doQueryPushShield: IMQueryPushShieldReq ParseFromArray failed!!!");
        }
    }
