...
namespace Flow{
    ...
    void CFileModel::getOfflineFile(uint32_t userId, std::list<IM::BaseDefine::OfflineFileInfo>& lsOffline)
    {
        CDBManager* pDBManager = CDBManager::getInstance();
        CDBConn* pDBConn = pDBManager->GetDBConn("teamtalk_slave");
        if (pDBConn)
        {
            std::string strSql = "select * from IMTransmitFile where toId="+Util::int2string(userId) + " and status=0 order by created";
            ...
            {
                LOG(ERROR)<<"no result for:" << strSql.c_str();
            }
    ...
    void CFileModel::addOfflineFile(uint32_t fromId, uint32_t toId, std::string& taskId, std::string& fileName, uint32_t fileSize)
    {
        CDBManager* pDBManager = CDBManager::getInstance();
        CDBConn* pDBConn = pDBManager->GetDBConn("teamtalk_master");
        if (pDBConn)
        {
            std::string strSql = "insert into IMTransmitFile (`fromId`,`toId`,`fileName`,`size`,`taskId`,`status`,`created`,`updated`) values(?,?,?,?,?,?,?,?)";
    ...
    void CFileModel::delOfflineFile(uint32_t fromId, uint32_t toId, std::string& taskId)
    {
        CDBManager* pDBManager = CDBManager::getInstance();
        CDBConn* pDBConn = pDBManager->GetDBConn("teamtalk_master");
        if (pDBConn)
        {
            std::string strSql = "delete from IMTransmitFile where  fromId=" + Util::int2string(fromId) + " and toId="+Util::int2string(toId) + " and taskId='" + taskId + "'";
            if(pDBConn->ExecuteUpdate(strSql.c_str()))
            {
                LOG(INFO)<<"delete offline file success."<<fromId<<"->"<<toId<<":"<<taskId;
            }
            else
            {
                LOG(ERROR)<<"delete offline file failed."<<fromId<<"->"<<toId<<":"<<taskId;
            }
            pDBManager->RelDBConn(pDBConn);
        }
        else
