...
namespace Flow::DB_PROXY {
    ...
    void getRecentSession(CImPdu *pPdu, uint32_t conn_uuid) {
        ...
        if (msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength())) {
            ...
            //获取最近联系人列表
            std::list<IM::BaseDefine::ContactSessionInfo> lsContactList;
            ...
            RAW_LOG(INFO, "userId=%u, last_time=%u, count=%u", nUserId, nLastTime, msgResp.contact_session_list_size());
    ...
    void deleteRecentSession(CImPdu *pPdu, uint32_t conn_uuid) {
        ...
        if (msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength())) {
            ...
            if (IM::BaseDefine::SessionType_IsValid(nType)) {
                ...
                RAW_LOG(INFO, "userId=%d, peerId=%d, result=%s", nUserId, nPeerId, bRet ? "success" : "failed");
                ...
                RAW_LOG(INFO, "invalied session_type. userId=%u, peerId=%u, seseionType=%u", nUserId, nPeerId, nType);
            }
        } else {
            RAW_LOG(INFO, "parse pb failed");
        }
