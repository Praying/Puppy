...
//======================================================================================================================
        ...
        BString::BString()
                : mString(0), mType(BSTRType_ANSI), mAllocated(0), mCharacterWidth(1), mLength(0)
        {
            _allocate();
...
//======================================================================================================================
        ...
        BString::BString(BStringType type, uint16_t length)
                : mString(0), mType(type), mAllocated(0), mLength(length)
        {
...
//======================================================================================================================
        ...
        BString::BString(const uint16_t *data)
                : mString(0), mType(BSTRType_Unicode16), mAllocated(0), mCharacterWidth(2), mLength(0)
        {
            _allocate();
            // we might get a null pointer from db queries
            if (data != NULL)
...
//======================================================================================================================
        ...
        uint16_t BString::initRawBSTR(char *data, BStringType type)
        {
            ...
            if (charLen > mAllocated)
            {
                ...
                mAllocated = (((static_cast<uint16_t>(charLen) / BSTRING_ALLOC_BLOCK_SIZE) + 1) *
...
//======================================================================================================================
        ...
        void BString::convert(BStringType type)
        {
            ...
            switch (type)
            {
                ...
                {
                    mCharacterWidth = 2;
                    allocated = (((((mLength + 1) * mCharacterWidth) / BSTRING_ALLOC_BLOCK_SIZE) + 1) *
                                 BSTRING_ALLOC_BLOCK_SIZE);
        ...
        void BString::substring(BString &dest, uint16_t start, uint16_t end)
        {
            ...
            switch (mType)
            {
                ...
                case BSTRType_Unicode16:
                {
                    ...
                    wcsncpy(reinterpret_cast<wchar_t *>(destBuffer), reinterpret_cast<wchar_t *>(mString) + start,
                            end - start);
                    break;
                }
            }
            ...
            if (mLength * mCharacterWidth >= mAllocated)
            {
                ...
                switch (mType)
                {
                    case BSTRType_ANSI:
                    {
                        ...
                        if (mString)
                        {
                            memcpy(newString, mString,
                                   std::min<uint32_t>(mLength + mCharacterWidth, previousAllocated));
                        }
                    }
                        break;
                    ...
                    case BSTRType_UTF8:
                    {
                        ...
                        {
                            memcpy(newString, mString,
                                   std::min<uint32_t>((mLength + mCharacterWidth) * 2, previousAllocated));
                        }
                    }
                        break;
                }
...
//======================================================================================================================
        uint32_t BString::mCrcTable[256] = {0x0000000, 0x04C11DB7, 0x09823B6E, 0x0D4326D9, 0x130476DC, 0x17C56B6B,
