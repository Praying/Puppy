    ...
    void imconn_callback(void* callback_data, uint8_t msg, uint32_t handle, void* pParam)
    {
        ...
        switch (msg)
        {
            ...
            case NETLIB_MSG_CLOSE:
                ...
                break;
            default:
                LOG(ERROR)<<"!!!imconn_callback error msg: " << msg;
                break;
        }
...
//	++g_send_pkt_cnt;
        ...
        if (remain > 0)
        {
            ...
            m_busy = true;
            LOG(INFO)<<"send busy, remain= "<< m_out_buf.GetWriteOffset();
        }
        else
        {
            OnWriteCompelete();
...
//			++g_recv_pkt_cnt;
            ...
            LOG(ERROR)<<"!!!catch exception, sid="<<ex.GetServiceId()<<", cid="<<ex.GetCommandId()<<", err_code="<<ex.GetErrorCode()<<", err_msg="<<ex.GetErrorMsg()<<", close the connection ";
            if (pPdu) {
                delete pPdu;
                pPdu = NULL;
            }
    ...
    void CImConn::OnWrite()
    {
        ...
        LOG(INFO)<<"onWrite, remain=%d "<<m_out_buf.GetWriteOffset();
    }
