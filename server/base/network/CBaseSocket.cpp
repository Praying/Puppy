...
#include "CBaseSocket.hpp"
#include "CEventDispatcher.hpp"
#include <map>
...
//////////////////////////////
    ...
    int CBaseSocket::Listen(const char *server_ip, uint16_t port, callback_t callback, void *callback_data) {
        ...
        if (ret == SOCKET_ERROR) {
            LOG(ERROR)<<"bind failed, err_code="<< _GetErrorCode();
        ...
        if (ret == SOCKET_ERROR) {
            LOG(ERROR)<<"listen failed, err_code="<< _GetErrorCode();
            closesocket(m_socket);
            return NETLIB_ERROR;
        }
        ...
        LOG(INFO)<<"CBaseSocket::Listen on "<<server_ip<<":"<< port;
    ...
    net_handle_t CBaseSocket::Connect(const char *server_ip, uint16_t port, callback_t callback, void *callback_data) {
        LOG(INFO)<<"CBaseSocket::Connect, server_ip="<<server_ip<<", port="<< port;
        ...
        m_socket = socket(AF_INET, SOCK_STREAM, 0);
        if (m_socket == INVALID_SOCKET) {
            LOG(ERROR)<<"socket failed, err_code="<< _GetErrorCode();
        ...
        if ((ret == SOCKET_ERROR) && (!_IsBlock(_GetErrorCode()))) {
            LOG(ERROR)<<"connect failed, err_code="<< _GetErrorCode();
            closesocket(m_socket);
            return NETLIB_INVALID_HANDLE;
        }
    ...
    int CBaseSocket::Send(void *buf, int len) {
        ...
        if (ret == SOCKET_ERROR) {
            ...
            if (_IsBlock(err_code)) {
                ...
                LOG(ERROR)<<"!!!send failed, error code: "<< err_code;
            }
        }
...
#endif
        if (ret == SOCKET_ERROR) {
            LOG(ERROR)<<"_SetNonblock failed, err_code=" <<_GetErrorCode();
        }
    }
    ...
    void CBaseSocket::_SetReuseAddr(SOCKET fd) {
        ...
        if (ret == SOCKET_ERROR) {
            LOG(ERROR)<<"_SetReuseAddr failed, err_code="<< _GetErrorCode();
        }
    }
    ...
    void CBaseSocket::_SetNoDelay(SOCKET fd) {
        ...
        if (ret == SOCKET_ERROR) {
            LOG(ERROR)<<"_SetNoDelay failed, err_code="<<_GetErrorCode();
        }
    }
    ...
    void CBaseSocket::_SetAddr(const char *ip, const uint16_t port, sockaddr_in *pAddr) {
        ...
        if (pAddr->sin_addr.s_addr == INADDR_NONE) {
            ...
            if (host == NULL) {
                LOG(ERROR)<<"gethostbyname failed, ip="<<ip;
                return;
            }
    ...
    void CBaseSocket::_AcceptNewSocket() {
        ...
        while ((fd = accept(m_socket, (sockaddr *) &peer_addr, &addr_len)) != INVALID_SOCKET) {
            ...
            LOG(ERROR)<<"AcceptNewSocket, socket="<<fd<<"from "<<ip_str<<":"<< port;
