...
//    curl_easy_setopt(curl, CURLOPT_POST, 1);
        ...
        if (CURLE_OK != res) {
            LOG(ERROR) << "curl_easy_perform failed, res=" << res;
            return "";
        }
        ...
        if (!reader.parse(strResp, value)) {
            LOG(ERROR) << "json parse failed: " << strResp.c_str();
            return "";
        }

        if (value["error_code"].isNull()) {
            LOG(ERROR) << "no code in response " << strResp.c_str();
            return "";
        }
        uint32_t nRet = value["error_code"].asUInt();
        if (nRet != 0) {
            LOG(ERROR) << "upload faile:" << nRet;
            return "";
        }
    ...
    bool CHttpClient::DownloadByteFile(const string &url, AudioMsgInfo *pAudioMsg) {
        ...
        if (CURLE_OK != res || retcode != 200) {
            LOG(ERROR) << "curl_easy_perform failed, res=" << res << ", ret=" << retcode;
        }
        double nLen = 0;
