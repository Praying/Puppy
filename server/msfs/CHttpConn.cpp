...
// conn_handle 从0开始递增，可以防止因socket handle重用引起的一些冲突
                                  ...
                                  void* pParam)
    {
        ...
        uint64_t cur_time = Util::get_tick_count();
...
//////////////////////////
    ...
    void CHttpTask::OnUpload()
    {
        ...
        char *pContent = NULL;
        int nTmpLen = 0;
        const char* pPos = Util::memfind(m_pContent, m_nContentLen, CONTENT_DISPOSITION, strlen(CONTENT_DISPOSITION));
        if (pPos != NULL)
        {
            nTmpLen = pPos - m_pContent;
            const char* pPos2 = Util::memfind(pPos, m_nContentLen - nTmpLen, "filename=", strlen("filename="));
            if (pPos2 != NULL)
            {
                pPos = pPos2 + strlen("filename=") + 1;
                const char * pPosQuotes = Util::memfind(pPos, m_nContentLen - nTmpLen, "\"", strlen("\""));
                ...
                if(nFileNameLen <= 255)
                {
                    ...
                    const char* pPosType = Util::memfind(szFileName, nFileNameLen, ".", 1, false);
                    if(pPosType != NULL)
                    {
                        char szType[16];
                        ...
                        if(nTypeLen <=15)
                        {
                            ...
                            if(pPosExtend != NULL)
                            {
                                const char* pPosTmp = Util::memfind(pPosExtend, nFileNameLen - (pPosExtend + 1 - szFileName), "x", 1);
                                if(pPosTmp != NULL)
                                {
                            ...
                            if (nPos != m_strContentType.npos)
                            {
                                ...
                                pPos = Util::memfind(m_pContent, m_nContentLen, pBoundary, nBoundaryLen);
                                if (NULL != pPos)
                                {
                                    nTmpLen = pPos - m_pContent;
                                    pPos = Util::memfind(m_pContent + nTmpLen, m_nContentLen - nTmpLen, CONTENT_TYPE, strlen(CONTENT_TYPE));
                                    if (NULL != pPos)
                                    {
                                        nTmpLen = pPos - m_pContent;
                                        pPos = Util::memfind(m_pContent + nTmpLen, m_nContentLen - nTmpLen, HTTP_END_MARK, strlen(HTTP_END_MARK));
                                        if (NULL != pPos)
                                        {
                                            nTmpLen = pPos - m_pContent;
                                            const char* pFileStart = pPos + strlen(HTTP_END_MARK);
                                            pPos2 = Util::memfind(m_pContent + nTmpLen, m_nContentLen - nTmpLen, pBoundary, nBoundaryLen);
                                            if (NULL != pPos2)
                                            {
    ...
    void  CHttpTask::OnDownload()
    {
        ...
        if(g_fileManager->getAbsPathByUrl(m_strUrl, strPath ) == 0)
        {
            ...
            if(nTmpSize != -1)
            {
                ...
                std::string strType = strPath.substr(nPos + 1, strPath.length() - nPos);
    ...
    CHttpConn::CHttpConn()
    {
        ...
        m_last_send_tick = m_last_recv_tick = Util::get_tick_count();
    ...
    {
        m_last_send_tick = Util::get_tick_count();
    ...
    void CHttpConn::OnRead()
    {
        for (;;)
        {
            ...
            m_last_recv_tick = Util::get_tick_count();
        }
