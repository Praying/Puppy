...
//////////////////////////
    CHttpConn::CHttpConn()
    {
        ...
        //RAW_LOG(ERROR,"CHttpConn, handle=%u\n", m_conn_handle);
    }
    ...
    int CHttpConn::Send(void* data, int len)
    {
        ...
        if (ret < len)
        {
            ...
            m_busy = true;
            //RAW_LOG(ERROR,"not send all, remain=%d\n", m_out_buf.GetWriteOffset());
        }
        else
        {
            OnWriteComlete();
    ...
    void CHttpConn::OnRead()
    {
        ...
        if(buf_len > 1024)
        {
            RAW_LOG(ERROR,"get too much data:%s ", in_buf);
            Close();
            return;
        }

        //RAW_LOG(ERROR,"OnRead, buf_len=%u, conn_handle=%u\n", buf_len, m_conn_handle); // for debug
        ...
        if (m_cHttpParser.IsReadAll()) {
            ...
            if (strncmp(url.c_str(), "/msg_server", 11) == 0) {
                ...
                RAW_LOG(ERROR,"url unknown, url=%s ", url.c_str());
                Close();
            }
        }
    }
...
// Add By Lanhu 2014-12-19 通过登陆IP来优选电信还是联通IP
    void CHttpConn::_HandleMsgServRequest(std::string& url, std::string& post_data)
    {
        ...
        std::map<uint32_t, msg_serv_info_t*>::iterator it_min_conn = g_msg_serv_info.end();
        std::map<uint32_t, msg_serv_info_t*>::iterator it;
        ...
        if (it_min_conn == g_msg_serv_info.end()) {
            ...
            {
                value["priorIP"] = std::string(it_min_conn->second->ip_addr1);
                value["backupIP"] = std::string(it_min_conn->second->ip_addr2);
                value["msfsPrior"] = strMsfsUrl;
                value["msfsBackup"] = strMsfsUrl;
            }
            else
            {
                value["priorIP"] = std::string(it_min_conn->second->ip_addr2);
                value["backupIP"] = std::string(it_min_conn->second->ip_addr1);
                value["msfsPrior"] = strMsfsUrl;
                value["msfsBackup"] = strMsfsUrl;
            }
