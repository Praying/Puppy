...
namespace Flow::LoginServer {
    ...
    void CLoginConn::Close() {
        if (m_handle != NETLIB_INVALID_HANDLE) {
            ...
            if (m_conn_type == LOGIN_CONN_TYPE_CLIENT) {
                ...
                std::map<uint32_t, msg_serv_info_t *>::iterator it = g_msg_serv_info.find(m_handle);
                if (it != g_msg_serv_info.end()) {
                    msg_serv_info_t *pMsgServInfo = it->second;

                    g_total_online_user_cnt -= pMsgServInfo->cur_conn_cnt;
                    RAW_LOG(ERROR, "onclose from MsgServer: %s:%u ", pMsgServInfo->hostname.c_str(),
                            pMsgServInfo->port);
                    delete pMsgServInfo;
    ...
    void CLoginConn::OnTimer(uint64_t curr_tick) {
        if (m_conn_type == LOGIN_CONN_TYPE_CLIENT) {
            ...
            if (curr_tick > m_last_recv_tick + SERVER_TIMEOUT) {
                RAW_LOG(ERROR, "connection to MsgServer timeout ");
                Close();
            }
        }
    }
    ...
    void CLoginConn::HandlePdu(CImPdu *pPdu) {
        switch (pPdu->GetCommandId()) {
            ...
            default:
                RAW_LOG(ERROR, "wrong msg, cmd id=%d ", pPdu->GetCommandId());
                break;
        }
    }
    ...
    void CLoginConn::_HandleMsgServInfo(CImPdu *pPdu) {
        ...
        g_msg_serv_info.insert(std::make_pair(m_handle, pMsgServInfo));

        g_total_online_user_cnt += pMsgServInfo->cur_conn_cnt;

        RAW_LOG(ERROR, "MsgServInfo, ip_addr1=%s, ip_addr2=%s, port=%d, max_conn_cnt=%d, cur_conn_cnt=%d, "\
        "hostname: %s. ",
    ...
    void CLoginConn::_HandleUserCntUpdate(CImPdu *pPdu) {
        ...
        if (it != g_msg_serv_info.end()) {
            ...
            RAW_LOG(ERROR, "%s:%d, cur_cnt=%u, total_cnt=%u ", pMsgServInfo->hostname.c_str(),
    ...
    void CLoginConn::_HandleMsgServRequest(CImPdu *pPdu) {
        ...
        RAW_LOG(ERROR, "HandleMsgServReq. ");
        ...
        std::map<uint32_t, msg_serv_info_t *>::iterator it_min_conn = g_msg_serv_info.end(), it;
        ...
        if (it_min_conn == g_msg_serv_info.end()) {
            RAW_LOG(ERROR, "All TCP MsgServer are full ");
            IM::Login::IMMsgServRsp msg;
