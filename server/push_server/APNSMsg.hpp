...
#ifndef SERVER_APNSMSG_HPP
#define SERVER_APNSMSG_HPP

#include "socket/CBaseMsg.hpp"
#include <string>
#include <list>
...
#define PACKED __attribute__( ( packed, aligned(1) ) )
    ...
    public:
        ...
        void SetDeviceToken(std::string strDeviceToken) { m_strDeviceToken = strDeviceToken; }
        const std::string& GetDeviceToken() { return m_strDeviceToken; }
        ...
        void SetCustomData(std::string strCustomData) { m_strCustomData = strCustomData; }
        const std::string& GetCustomData() { return m_strCustomData; }
        void SetBadge(uint32_t nBadge) { m_nBadge = nBadge; }
        uint32_t GetBadge() { return m_nBadge; }
        void SetAlterBody(std::string strAlterBody) { m_strAlterBody = strAlterBody; }
        const std::string& GetAlterBody() { return m_strAlterBody; }
        void SetActionLocKey(std::string strActionLocKey) { m_strActionLocKey = strActionLocKey; }
        const std::string& GetActionLocKey() { return m_strActionLocKey; }
        void SetLocKey(std::string strLocKey) { m_strLocKey = strLocKey; }
        const std::string& GetLocKey() { return m_strLocKey; }
        void SetLocArgsList(std::list<std::string>& LocArgsList) { m_LocArgsList = LocArgsList; }
        const std::list<std::string>& GetLocArgsList() { return m_LocArgsList; }
        void SetLaunchImage(std::string strLaunchImage) { m_strLaunchImage = strLaunchImage; }
        const std::string& GetLaunchImage() { return m_strLaunchImage; }
    private:
    ...
    private:
        ...
        BOOL            m_bSound;
        uint32_t        m_nBadge;
        std::string          m_strCustomData;
        /* payload alter */
        std::string          m_strAlterBody;
        std::string          m_strActionLocKey;
        std::string          m_strLocKey;
        std::list<std::string>    m_LocArgsList;
        std::string          m_strLaunchImage;
    };
