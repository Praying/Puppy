...
*/
    void CIOLoop::Run()
    {
        ...
        while (m_bCloseRequest == FALSE)
        {
            ...
            m_MapMutex.Lock();
            std::map<S_SOCKET, CBaseIOStream*> mapTmp = m_MapIOStreamBySocket;
            m_MapMutex.Unlock();
            std::map<S_SOCKET, CBaseIOStream*>::iterator it = mapTmp.begin();
            ...
            if (nRet > 0)
            {
                ...
                std::map<S_SOCKET, CBaseIOStream*>::iterator it1 = mapTmp.begin();
...
*/
    CBaseIOStream* CIOLoop::_GetHandlerBySock( S_SOCKET sock )
    {
        ...
        std::map<S_SOCKET, CBaseIOStream*>::iterator it = m_MapIOStreamBySocket.find(sock);
