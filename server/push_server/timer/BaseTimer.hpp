...
#ifndef SERVER_BASETIMER_HPP
#define SERVER_BASETIMER_HPP
...
#include <utility>
#include <push_server/thread/BaseThread.hpp>
...
public:
    ...
    static void* ThreadProc(void* pParam)
    {
        ...
        while (pInstance->m_iQuit == 0)
        {
            ...
            std::map<int, pair<unsigned int, unsigned int > > tmp = pInstance->m_MapInterval;
            std::map<int, void*> paramtmp = pInstance->m_MapParam;
            pInstance->m_MapMutex.Unlock();

            std::map<int, pair<unsigned int, unsigned int > >::iterator iter = tmp.begin();
            while (iter != tmp.end())
            {
                ...
                if (iter->second.second >= iter->second.first)
                {
                    ...
                    pInstance->m_MapMutex.Lock();
                    std::map<int, pair<unsigned int, unsigned int > >::iterator it = pInstance->m_MapInterval.find(iter->first);
...
public:
    ...
    std::map<int/* nIndex */, pair<unsigned int/* time of miliseconds 预设定时器的时间 */ , unsigned int/* time of miliseconds，当前时间 */> > m_MapInterval;
    std::map<int/* nIndex */, void* > m_MapParam;
