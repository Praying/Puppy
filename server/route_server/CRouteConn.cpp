...
namespace Flow{
    ...
    {
        LOG(ERROR)<<"MsgServer onclose: handle= "<< m_handle;
        Close();
    }
    ...
    void CRouteConn::OnTimer(uint64_t curr_tick)
    {
        ...
        if (curr_tick > m_last_recv_tick + SERVER_TIMEOUT) {
            LOG(ERROR)<<"message server timeout ";
            Close();
        }
    }
    ...
    void CRouteConn::HandlePdu(CImPdu* pPdu)
    {
        switch (pPdu->GetCommandId()) {
            ...
            default:
                LOG(ERROR)<<"CRouteConn::HandlePdu, wrong cmd id:  " << pPdu->GetCommandId();
                break;
        }
    }
    ...
    void CRouteConn::_HandleOnlineUserInfo(CImPdu* pPdu)
    {
        ...
        LOG(ERROR)<<"HandleOnlineUserInfo, user_cnt= " << user_count;
    ...
    void CRouteConn::_HandleUserStatusUpdate(CImPdu* pPdu)
    {
        ...
        LOG(ERROR)<<"HandleUserStatusUpdate, status="<<user_status<<", uid="<<user_id<<", client_type="<< client_type;
    ...
    void CRouteConn::_HandleRoleSet(CImPdu* pPdu)
    {
        ...
        LOG(ERROR)<<"HandleRoleSet, master="<<master<<", handle="<<m_handle;
        if (master == 1) {
            m_bMaster = true;
        } else {
            m_bMaster = false;
    ...
    void CRouteConn::_HandleUsersStatusRequest(CImPdu* pPdu)
    {
        ...
        LOG(ERROR)<<"HandleUserStatusReq, req_id="<<request_id<<", query_count="<< query_count;
