...
#include <push_server/PushDefine.hpp>
...
namespace Flow::MsgServer {
    ...
    void init_db_serv_conn(serv_info_t *server_list, uint32_t server_count, uint32_t concur_conn_cnt) {
        ...
        LOG(ERROR) << "DB server connection index for login business: [0, " << g_db_server_login_count
                   << "), for other business: [" << g_db_server_login_count << ", " << g_db_server_count << ") ";
...
// get a random db server connection in the range [start_pos, stop_pos)
    ...
    void CDBServConn::Connect(const char *server_ip, uint16_t server_port, uint32_t serv_idx) {
        LOG(ERROR) << "Connecting to DB Storage Server  " << server_ip << ":" << server_port;
    ...
    void CDBServConn::OnConfirm() {
        LOG(ERROR) << "connect to db server success";
        m_bOpen = true;
        g_db_server_list[m_serv_idx].reconnect_cnt = MIN_RECONNECT_CNT / 2;
    }

    void CDBServConn::OnClose() {
        LOG(ERROR) << "onclose from db server handle=" << m_handle;
        Close();
    }
    ...
    void CDBServConn::OnTimer(uint64_t curr_tick) {
        ...
        if (curr_tick > m_last_recv_tick + SERVER_TIMEOUT) {
            LOG(ERROR) << "conn to db server timeout";
            Close();
        }
    }
    ...
    void CDBServConn::HandlePdu(CImPdu *pPdu) {
        switch (pPdu->GetCommandId()) {
            ...
            default:
                LOG(ERROR) << "db server, wrong cmd id= " << pPdu->GetCommandId();
        }
    }
    ...
    void CDBServConn::_HandleValidateResponse(CImPdu *pPdu) {
        ...
        std::string login_name = msg.user_name();
        uint32_t result = msg.result_code();
        std::string result_string = msg.result_string();
        CDbAttachData attach_data((uchar_t *) msg.attach_data().c_str(), msg.attach_data().length());
        LOG(ERROR) << "HandleValidateResp, user_name=" << login_name << ", result=" << result;

        CImUser *pImUser = CImUserManager::GetInstance()->GetImUserByLoginName(login_name);
        CMsgConn *pMsgConn = NULL;
        if (!pImUser) {
            LOG(ERROR) << "ImUser for user_name=" << login_name << " not exist";
            return;
        } else {
            pMsgConn = pImUser->GetUnValidateMsgConn(attach_data.GetHandle());
            if (!pMsgConn || pMsgConn->IsOpen()) {
                LOG(ERROR) << "no such conn is validated, user_name=" << login_name;
                return;
            }
        }
        ...
        if (result == 0) {
            ...
            pUser->KickOutSameClientType(pMsgConn->GetClientType(), IM::BaseDefine::KICK_REASON_DUPLICATE_USER,
                                         pMsgConn);
            ...
            LOG(ERROR) << "user_name: " << user_id << ", uid: " << user_id;
    ...
    void CDBServConn::_HandleRecentSessionResponse(CImPdu *pPdu) {
        ...
        LOG(ERROR) << "HandleRecentSessionResponse, userId=" << user_id << ", session_cnt=" << session_cnt;
    ...
    void CDBServConn::_HandleAllUserResponse(CImPdu *pPdu) {
        ...
        LOG(INFO) << "HandleAllUserResponse, userId=" << user_id << ", latest_update_time=" << latest_update_time
                  << ", user_cnt=" << user_cnt;
    ...
    void CDBServConn::_HandleGetMsgListResponse(CImPdu *pPdu) {
        ...
        LOG(INFO) << "HandleGetMsgListResponse, userId=" << user_id << ", session_type=" << session_type
                  << ", opposite_user_id=" << session_id << ", msg_id_begin=" << msg_id_begin << ", cnt=" << msg_cnt;
    ...
    void CDBServConn::_HandleGetMsgByIdResponse(CImPdu *pPdu) {
        ...
        LOG(ERROR) << "HandleGetMsgByIdResponse, userId=" << user_id << ", session_type=" << session_type
                   << ", opposite_user_id=" << session_id << ", cnt=" << msg_cnt;
    ...
    void CDBServConn::_HandleMsgData(CImPdu *pPdu) {
        ...
        if (msg_id == 0) {
            LOG(ERROR) << "HandleMsgData, write db failed" << from_user_id << ":" << to_user_id;
            return;
        }
    ...
    void CDBServConn::_HandleGetLatestMsgIDRsp(CImPdu *pPdu) {
        ...
        LOG(INFO) << "HandleUnreadMsgCntResp, userId=" << user_id << ", session_id=" << session_id << ", session_type="
                  << session_type << ", latest_msg_id=" << latest_msg_id;
    ...
    void CDBServConn::_HandleUnreadMsgCountResponse(CImPdu *pPdu) {
        ...
        LOG(INFO) << "HandleUnreadMsgCntResp, userId=" << user_id << ", total_cnt=" << total_cnt << ", user_unread_cnt="
                  << user_unread_cnt;
    ...
    void CDBServConn::_HandleUsersInfoResponse(CImPdu *pPdu) {
        ...
        LOG(INFO) << "HandleUsersInfoResp, user_id=" << user_id << ", user_cnt=" << user_cnt;
    ...
    void CDBServConn::_HandleStopReceivePacket(CImPdu *pPdu) {
        LOG(ERROR) << "HandleStopReceivePacket, from " <<
    ...
    void CDBServConn::_HandleRemoveSessionResponse(CImPdu *pPdu) {
        ...
        uint32_t session_type = msg.session_type();
        uint32_t session_id = msg.session_id();
        LOG(ERROR) << "HandleRemoveSessionResp, req_id=" << user_id << ", result=" << result << ", session_id="
    ...
    void CDBServConn::_HandleSetDeviceTokenResponse(CImPdu *pPdu) {
        ...
        uint32_t user_id = msg.user_id();
        LOG(ERROR) << "HandleSetDeviceTokenResponse, user_id = " << user_id;
    }
    ...
    void CDBServConn::_HandleGetDeviceTokenResponse(CImPdu *pPdu) {
        ...
        if (msg_type == IM::BaseDefine::MSG_TYPE_SINGLE_TEXT || msg_type == IM::BaseDefine::MSG_TYPE_GROUP_TEXT) {
            ...
            if (pAes->Decrypt(msg_data.c_str(), msg_data.length(), &msg_out, msg_out_len) == 0) {
                msg_data = std::string(msg_out, msg_out_len);
            } else {
                LOG(ERROR) << "HandleGetDeviceTokenResponse, decrypt msg failed, from_id:" << from_id << ", to_id: "
                           << to_id << ", msg_type: " << msg_type;
                return;
            }
            pAes->Free(msg_out);
        }
        ...
        LOG(INFO) << "HandleGetDeviceTokenResponse, user_token_cnt = " << user_token_cnt;
        ...
        for (uint32_t i = 0; i < user_token_cnt; i++) {
            ...
            std::string device_token = user_token.token();
            ...
            LOG(ERROR) << "HandleGetDeviceTokenResponse, user_id = " << user_id << ", device_token = " << device_token
                       << ", push_cnt = " << push_cnt << ", client_type = " << client_type;
            ...
            if (pUser) {
                ...
                if (pUser->GetPCLoginStatus() == IM_PC_LOGIN_STATUS_ON) {
                    ...
                    LOG(INFO) << "HandleGetDeviceTokenResponse, user id: " << user_id << " push type: silent.";
                } else {
                    user_token_tmp->set_push_type(IM_PUSH_TYPE_NORMAL);
                    LOG(INFO) << "HandleGetDeviceTokenResponse, user id: " << user_id << ", push type: normal.";
                }
            } else {
    ...
    void CDBServConn::_HandleChangeSignInfoResponse(CImPdu *pPdu) {
        ...
        LOG(INFO) << "HandleChangeSignInfoResp: user_id=" << user_id << ", result=" << result;
        ...
        if (pMsgConn && pMsgConn->IsOpen()) {
            ...
            LOG(ERROR) << "HandleChangeSignInfoResp: can't found msg_conn by user_id = " << user_id << ", handle = "
                       << handle;
        ...
        if (!result) {
            ...
            if (route_conn) {
                ...
                LOG(ERROR) << "HandleChangeSignInfoResp: can't found route_conn";
            }
        }
    }
    ...
    void CDBServConn::_HandlePushShieldResponse(CImPdu *pPdu) {
        ...
        LOG(INFO) << "_HandlePushShieldResponse: user_id=" << user_id << ", result=" << result;
        ...
        if (pMsgConn && pMsgConn->IsOpen()) {
            ...
            LOG(ERROR) << "_HandlePushShieldResponse: can't found msg_conn by user_id = " << user_id << ", handle = "
                       << handle;
        }
    }
    ...
    void CDBServConn::_HandleQueryPushShieldResponse(CImPdu *pPdu) {
        ...
        LOG(INFO) << "_HandleQueryPushShieldResponse: user_id=" << user_id << ", result=" << result;
        ...
        if (pMsgConn && pMsgConn->IsOpen()) {
            ...
            LOG(ERROR) << "_HandleQueryPushShieldResponse: can't found msg_conn by user_id = " << user_id
                       << ", handle = " << handle;
        }
    }
