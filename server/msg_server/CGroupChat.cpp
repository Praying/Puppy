...
namespace Flow::MsgServer {
    ...
    void CGroupChat::HandleClientGroupNormalRequest(CImPdu *pPdu, CMsgConn *pFromConn) {
        ...
        LOG(ERROR) << "HandleClientGroupNormalRequest, user_id= " << user_id;
    ...
    void CGroupChat::HandleGroupNormalResponse(CImPdu *pPdu) {
        ...
        LOG(ERROR) << "HandleGroupNormalResponse, user_id=" << user_id << ", group_cnt= " << group_cnt;
    ...
    void CGroupChat::HandleClientGroupInfoRequest(CImPdu *pPdu, CMsgConn *pFromConn) {
        ...
        uint32_t user_id = pFromConn->GetUserId();
        uint32_t group_cnt = msg.group_version_list_size();
        LOG(INFO) << "HandleClientGroupInfoRequest, user_id=" << user_id << ", group_cnt= " << group_cnt;
    ...
    void CGroupChat::HandleGroupInfoResponse(CImPdu *pPdu) {
        ...
        LOG(INFO) << "HandleGroupInfoResponse, user_id=" << user_id << ", group_cnt=" << group_cnt;
        ...
        if (pduAttachData.GetPduLength() > 0 && group_cnt > 0) {
            ...
            LOG(ERROR) << "GroupInfoRequest is send by server, group_id=" << group_id;
            ...
            if (group_member_set.find(user_id) == group_member_set.end()) {
                LOG(ERROR) << "user_id=" << user_id << " is not in group, group_id=" << group_id;
                return;
            }
    ...
    void CGroupChat::HandleGroupMessage(CImPdu *pPdu) {
        ...
        if (msg_id == 0) {
            LOG(ERROR) << "HandleGroupMsg, write db failed, " << from_user_id << "->" << to_group_id;
            return;
        }
        uint8_t msg_type = msg.msg_type();
        CDbAttachData attach_data((uchar_t *) msg.attach_data().c_str(), msg.attach_data().length());

        LOG(ERROR) << "HandleGroupMsg, " << from_user_id << "->" << to_group_id << ", msg id=" << msg_id;
        ...
        CPduAttachData pduAttachData(ATTACH_TYPE_HANDLE_AND_PDU, attach_data.GetHandle(), pPdu->GetBodyLength(),
                                     pPdu->GetBodyData());
    ...
    void CGroupChat::HandleGroupMessageBroadcast(CImPdu *pPdu) {
        ...
        std::string msg_data = msg.msg_data();
        uint32_t msg_id = msg.msg_id();
        LOG(INFO) << "HandleGroupMessageBroadcast, " << from_user_id << "->" << to_group_id << ", msg id=" << msg_id;
    ...
    void CGroupChat::HandleClientGroupCreateRequest(CImPdu *pPdu, CMsgConn *pFromConn) {
        ...
        if (group_type == IM::BaseDefine::GROUP_TYPE_NORMAL) {
            LOG(ERROR) << "HandleClientGroupCreateRequest, create normal group failed, req_id=" << req_user_id
                       << ", group_name=" << group_name;
            return;
        }
        std::string group_avatar = msg.group_avatar();
        uint32_t user_cnt = msg.member_id_list_size();
        LOG(ERROR) << "HandleClientGroupCreateRequest, req_id=" << req_user_id << ", group_name=" << group_name
                   << ", avatar_url=" << group_avatar << ", user_cnt=" << user_cnt;
    ...
    void CGroupChat::HandleGroupCreateResponse(CImPdu *pPdu) {
        ...
        LOG(INFO) << "HandleGroupCreateResponse, req_id=" << user_id << ", result=" << result << ", group_id="
                  << group_id << ", group_name=" << group_name << ", member_cnt=" << user_cnt;
    ...
    void CGroupChat::HandleClientGroupChangeMemberRequest(CImPdu *pPdu, CMsgConn *pFromConn) {
        ...
        uint32_t group_id = msg.group_id();
        uint32_t user_cnt = msg.member_id_list_size();
        LOG(INFO) << "HandleClientChangeMemberReq, change_type=" << change_type << ", req_id=" << req_user_id
                  << ", group_id=" << group_id << ", user_cnt=" << user_cnt;
    ...
    void CGroupChat::HandleGroupChangeMemberResponse(CImPdu *pPdu) {
        ...
        LOG(ERROR)
                << "HandleChangeMemberResp, change_type=" << change_type << ", req_id=" << user_id << ", group_id="
                << group_id << ", result=" << result << ", chg_usr_cnt=" << chg_user_cnt << ", cur_user_cnt="
                << cur_user_cnt;
    ...
    void CGroupChat::HandleGroupChangeMemberBroadcast(CImPdu *pPdu) {
        ...
        uint32_t cur_user_cnt = msg.cur_user_id_list_size();
        LOG(ERROR)
                << "HandleChangeMemberBroadcast, change_type=" << change_type << ", group_id=" << group_id
                << ", chg_user_cnt=" << chg_user_cnt << ", cur_user_cnt=" << cur_user_cnt;
    ...
    void CGroupChat::HandleClientGroupShieldGroupRequest(CImPdu *pPdu, CMsgConn *pFromConn) {
        ...
        uint32_t group_id = msg.group_id();
        uint32_t user_id = pFromConn->GetUserId();
        LOG(ERROR) << "HandleClientGroupShieldGroupRequest, user_id: " << user_id << ", group_id: " << group_id
    ...
    void CGroupChat::HandleGroupShieldGroupResponse(CImPdu *pPdu) {
        ...
        uint32_t user_id = msg.user_id();
        uint32_t group_id = msg.group_id();
        LOG(INFO) << "HandleGroupShieldGroupResponse, result: " << result << ", user_id: " << user_id << ", group_id: "
                  << group_id;
    ...
    void CGroupChat::HandleGroupGetShieldByGroupResponse(CImPdu *pPdu) {
        ...
        LOG(INFO) << "HandleGroupGetShieldByGroupResponse, shield_status_list_cnt:  " << shield_status_list_cnt;
        ...
        for (uint32_t i = 0; i < shield_status_list_cnt; i++) {
            ...
            if (shield_status.shield_status() == 0) {
                ...
                LOG(ERROR)
                        << "user_id: " << shield_status.user_id() << " shield group, group id:  "
                        << shield_status.group_id();
            }
        }
        CImPdu pdu;
        pdu.SetPBMsg(&msg2);
