...
#include <base/proto/ProtosGen/IM.SwitchService.pb.h>
...
////////////////////////////
    ...
    void CMsgConn::OnConnect(net_handle_t handle) {
        ...
        g_msg_conn_map.insert(std::make_pair(handle, this));
    ...
    void CMsgConn::OnTimer(uint64_t curr_tick) {
        ...
        if (CHECK_CLIENT_TYPE_MOBILE(GetClientType())) {
            if (curr_tick > m_last_recv_tick + MOBILE_CLIENT_TIMEOUT) {
                LOG(ERROR) << "mobile client timeout, handle=" << m_handle << ", uid=" << GetUserId();
                Close();
                return;
            }
        } else {
            if (curr_tick > m_last_recv_tick + CLIENT_TIMEOUT) {
                LOG(ERROR) << "client timeout, handle=" << m_handle << ", uid= " << GetUserId();
                Close();
                return;
            }
        }
        ...
        if (!IsOpen()) {
            if (curr_tick > m_login_time + TIMEOUT_WATI_LOGIN_RESPONSE) {
                LOG(ERROR) << "login timeout, handle=" << m_handle << ", uid=" << GetUserId();
                Close();
                return;
            }
        }

        std::list<msg_ack_t>::iterator it_old;
        for (std::list<msg_ack_t>::iterator it = m_send_msg_list.begin(); it != m_send_msg_list.end();) {
            msg_ack_t msg = *it;
            it_old = it;
            it++;
            if (curr_tick >= msg.timestamp + TIMEOUT_WAITING_MSG_DATA_ACK) {
                LOG(ERROR) << "!!!a msg missed, msg_id=" << msg.msg_id << ", " << msg.from_id << "-> " << GetUserId();
                g_down_msg_miss_cnt++;
...
// process: send validate request to db server
    ...
    void CMsgConn::_HandleClientGetMsgListRequest(CImPdu *pPdu) {
        ...
        LOG(INFO) << "HandleClientGetMsgListRequest, req_id=" << GetUserId() << ", session_type=" << session_type
                  << ", session_id=" << session_id << " msg_id_begin=" << msg_id_begin << ", msg_cnt=" << msg_cnt;
        CDBServConn *pDBConn = get_db_serv_conn_for_login();
        if (pDBConn) {
    ...
    void CMsgConn::_HandleClientP2PCmdMsg(CImPdu *pPdu) {
        ...
        CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
        std::string cmd_msg = msg.cmd_msg_data();
    ...
    void CMsgConn::AddToSendList(uint32_t msg_id, uint32_t from_id) {
        //LOG(ERROR)<<"AddSendMsg, seq_no=%u, from_id=%u ", seq_no, from_id);
        msg_ack_t msg;
        msg.msg_id = msg_id;
        msg.from_id = from_id;
    ...
    void CMsgConn::DelFromSendList(uint32_t msg_id, uint32_t from_id) {
        //LOG(ERROR)<<"DelSendMsg, seq_no=%u, from_id=%u ", seq_no, from_id);
        for (std::list<msg_ack_t>::iterator it = m_send_msg_list.begin(); it != m_send_msg_list.end(); it++) {
            msg_ack_t msg = *it;
