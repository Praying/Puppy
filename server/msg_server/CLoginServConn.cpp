...
namespace Flow::MsgServer {
    ...
    static std::string g_msg_server_ip_addr1;
    static std::string g_msg_server_ip_addr2;
...
// if there is one LoginServer available, return true
    ...
    {
        LOG(ERROR)<<"Connecting to LoginServer "<<server_ip <<":"<< server_port;
        m_serv_idx = serv_idx;
        m_handle = netlib_connect(server_ip, server_port, imconn_callback, (void*)&g_login_server_conn_map);

        if (m_handle != NETLIB_INVALID_HANDLE) {
            g_login_server_conn_map.insert(std::make_pair(m_handle, this));
        }
    }
    ...
    {
        LOG(ERROR)<<"connect to login server success ";
        m_bOpen = true;
    ...
    {
        LOG(INFO)<<"login server conn onclose, from handle=" << m_handle;
        Close();
    }
    ...
    void CLoginServConn::OnTimer(uint64_t curr_tick)
    {
        ...
        if (curr_tick > m_last_recv_tick + SERVER_TIMEOUT) {
            LOG(ERROR)<<"conn to login server timeout ";
            Close();
        }
    }
