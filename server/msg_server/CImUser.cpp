...
namespace Flow::MsgServer {
    ...
    CImUser::CImUser(std::string user_name) {
        //log("ImUser, userId=%u\n", user_id);
    ...
    CMsgConn *CImUser::GetUnValidateMsgConn(uint32_t handle) {
        for (std::set<CMsgConn *>::iterator it = m_unvalidate_conn_set.begin();
    ...
    CMsgConn *CImUser::GetMsgConn(uint32_t handle) {
        ...
        std::map<uint32_t, CMsgConn *>::iterator it = m_conn_map.find(handle);
    ...
    user_conn_t CImUser::GetUserConn() {
        uint32_t conn_cnt = 0;
        for (std::map<uint32_t, CMsgConn *>::iterator it = m_conn_map.begin(); it != m_conn_map.end(); it++) {
    ...
    void CImUser::HandleKickUser(CMsgConn *pConn, uint32_t reason) {
        std::map<uint32_t, CMsgConn *>::iterator it = m_conn_map.find(pConn->GetHandle());
        if (it != m_conn_map.end()) {
            CMsgConn *pConn = it->second;
            if (pConn) {
                LOG(INFO) << "kick service user, user_id=" << m_user_id;
...
// 只支持一个WINDOWS/MAC客户端登陆,或者一个ios/android登录
    ...
    uint32_t CImUser::GetClientTypeFlag() {
        ...
        std::map<uint32_t, CMsgConn *>::iterator it = m_conn_map.begin();
    ...
    CImUser *CImUserManager::GetImUserByLoginName(std::string login_name) {
        CImUser *pUser = NULL;
    ...
    bool CImUserManager::AddImUserByLoginName(std::string login_name, CImUser *pUser) {
        bool bRet = false;
    ...
    void CImUserManager::RemoveImUserByLoginName(std::string login_name) {
    ...
    void CImUserManager::GetOnlineUserInfo(std::list<user_stat_t> *online_user_info) {
        user_stat_t status;
        ...
        for (ImUserMap_t::iterator it = m_im_user_map.begin(); it != m_im_user_map.end(); it++) {
            ...
            if (pImUser->IsValidate()) {
                std::map<uint32_t, CMsgConn *> &ConnMap = pImUser->GetMsgConnMap();
                for (std::map<uint32_t, CMsgConn *>::iterator it = ConnMap.begin(); it != ConnMap.end(); it++) {
                    CMsgConn *pConn = it->second;
                    if (pConn->IsOpen()) {
    ...
    void CImUserManager::GetUserConnCnt(std::list<user_conn_t> *user_conn_list, uint32_t &total_conn_cnt) {
        total_conn_cnt = 0;
        CImUser *pImUser = NULL;
        for (ImUserMap_t::iterator it = m_im_user_map.begin(); it != m_im_user_map.end(); it++) {
            pImUser = (CImUser *) it->second;
