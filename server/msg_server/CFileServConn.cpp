...
//
    ...
    void CFileServConn::Connect(const char *server_ip, uint16_t server_port, uint32_t idx) {
        LOG(ERROR) << "Connecting to FileServer " << server_ip << ":" << server_port;
    ...
    void CFileServConn::OnConfirm() {
        LOG(ERROR) << "connect to file server success ";
        m_bOpen = true;
    ...
    void CFileServConn::OnClose() {
        LOG(ERROR) << "onclose from file server handle= " << m_handle;
        Close();
    }
    ...
    void CFileServConn::OnTimer(uint64_t curr_tick) {
        ...
        if (curr_tick > m_last_recv_tick + SERVER_TIMEOUT) {
            LOG(ERROR) << "conn to file server timeout ";
            Close();
        }
    }
    ...
    void CFileServConn::_HandleFileMsgTransRsp(CImPdu *pPdu) {
        ...
        LOG(ERROR) << "HandleFileMsgTransRsp, result: " << result << " , from_user_id: " << from_id << ", to_user_id: "
                   << to_id << ", file_name: " << file_name << ",task_id:" << task_id << ",trans_mode:" << trans_mode;

        const std::list<IM::BaseDefine::IpAddr> *ip_addr_list = GetFileServerIPList();
        ...
        for (std::list<IM::BaseDefine::IpAddr>::const_iterator it = ip_addr_list->begin();
    ...
    void CFileServConn::_HandleFileServerIPRsp(CImPdu *pPdu) {
        ...
        for (uint32_t i = 0; i < ip_addr_cnt; i++) {
            ...
            LOG(INFO) << "_HandleFileServerIPRsp -> " << ip_addr.ip() << " : " << ip_addr.port();
