...
namespace Flow::MsgServer {
    ...
    void CFileHandler::HandleClientFileRequest(CMsgConn *pMsgConn, CImPdu *pPdu) {
        ...
        LOG(ERROR) << "HandleClientFileRequest, " << from_id << "->" << to_id << ", fileName: " << file_name
        ...
        if (pFileConn) {
            ...
            } else //IM::BaseDefine::FILE_TYPE_ONLINE
            {
                ...
                } else//无对应用户的pc登录状态,向route_server查询状态
                {
                    ...
                    CPduAttachData attach_data(ATTACH_TYPE_HANDLE_AND_PDU_FOR_FILE, pMsgConn->GetHandle(),
            ...
            LOG(ERROR) << "HandleClientFileRequest, no file server.   ";
    ...
    void CFileHandler::HandleClientFileHasOfflineReq(CMsgConn *pMsgConn, CImPdu *pPdu) {
        ...
        LOG(INFO) << "HandleClientFileHasOfflineReq, req_id=   " <<req_user_id;
        ...
        if (pDbConn) {
            ...
            LOG(WARNING) << "warning no DB connection available ";
    ...
    void CFileHandler::HandleClientFileAddOfflineReq(CMsgConn *pMsgConn, CImPdu *pPdu) {
        ...
        std::string file_name = msg.file_name();
        uint32_t file_size = msg.file_size();
        LOG(INFO) << "HandleClientFileAddOfflineReq, " << from_id << "->" << to_id << ", task_id: " << task_id
        ...
        CFileServConn *pFileConn = get_random_file_serv_conn();
        if (pFileConn) {
            const std::list<IM::BaseDefine::IpAddr> *file_addr_list = pFileConn->GetFileServerIPList();
            ...
            for (std::list<IM::BaseDefine::IpAddr>::const_iterator it = file_addr_list->begin();
    ...
    void CFileHandler::HandleClientFileDelOfflineReq(CMsgConn *pMsgConn, CImPdu *pPdu) {
        ...
        uint32_t to_id = msg.to_user_id();
        std::string task_id = msg.task_id();
        LOG(INFO) << "HandleClientFileDelOfflineReq, "<<from_id<<"->"<<to_id<<", task_id= "<< task_id.c_str();
    ...
    void CFileHandler::HandleFileHasOfflineRes(CImPdu *pPdu) {
        ...
        LOG(ERROR) << "HandleFileHasOfflineRes, req_id="<<req_user_id<<", file_cnt="<<file_cnt;

        CMsgConn *pConn = CImUserManager::GetInstance()->GetMsgConnByHandle(req_user_id,
                                                                            attach.GetHandle());
        CFileServConn *pFileConn = get_random_file_serv_conn();
        const std::list<IM::BaseDefine::IpAddr> *ip_list = NULL;
        if (pFileConn) {
            ip_list = pFileConn->GetFileServerIPList();
            for (std::list<IM::BaseDefine::IpAddr>::const_iterator it = ip_list->begin(); it != ip_list->end(); it++) {
                IM::BaseDefine::IpAddr ip_addr_tmp = *it;
            ...
            LOG(ERROR) << "HandleFileHasOfflineRes, no file server. ";
        }
        if (pConn) {
            pPdu->SetPBMsg(&msg);
            pConn->SendPdu(pPdu);
    ...
    void CFileHandler::HandleFileNotify(CImPdu *pPdu) {
        ...
        LOG(INFO) << "HandleFileNotify, from_id: " << from_user_id << ", to_id: " << to_user_id << " file_name: "
                  << file_name << ", task_id: " << task_id << ", trans_mode: " << trans_mode << "offline_ready:"
                  << offline_ready;
