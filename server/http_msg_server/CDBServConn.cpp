...
#define SERVER_TIMEOUT				30000
        ...
        void init_db_serv_conn(serv_info_t* server_list, uint32_t server_count, uint32_t concur_conn_cnt)
        {
            ...
            LOG(ERROR)<<"DB server connection index for login business: [0, "<<g_db_server_login_count<<"), for other business: ["<<g_db_server_login_count<<", "<<g_db_server_count<<")";
...
// get a random db server connection in the range [start_pos, stop_pos)
        ...
        {
            LOG(ERROR)<<"HandleStopReceivePacket, from "<<g_db_server_list[m_serv_idx].server_ip.c_str()<<":"<<g_db_server_list[m_serv_idx].server_port;
        ...
        void CDBServConn::_HandleCreateGroupRsp(CImPdu *pPdu)
        {
           ...
           // if (msg.has_group_id()) {
            ...
            if(!pHttpConn)
            {
                LOG(ERROR)<<"no http connection";
                return;
            }
            LOG(ERROR)<<"HandleCreateGroupRsp, req_id="<<user_id<<", group_name="<<group_name<<", result="<<result_code;
        ...
        void CDBServConn::_HandleChangeMemberRsp(CImPdu *pPdu)
        {
            ...
            LOG(ERROR)<<"HandleChangeMemberResp, change_type="<<change_type<<", req_id="<<user_id<<", group_id="<<group_id<<", result="<<result<<", chg_usr_cnt="<<chg_user_cnt<<", cur_user_cnt="<<cur_user_cnt;
            CDbAttachData attach_data((uchar_t*)msg.attach_data().c_str(), msg.attach_data().length());
            uint32_t http_handle = attach_data.GetHandle();
            CHttpConn* pHttpConn = FindHttpConnByHandle(http_handle);
            if(!pHttpConn)
