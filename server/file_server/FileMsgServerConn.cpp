...
namespace Flow::FileServer {
   // using namespace Util;
    ...
    void FileMsgServerConn::Close() {
        RAW_LOG(ERROR, "Close client, handle %d", m_handle);
    ...
    void FileMsgServerConn::OnClose() {
        RAW_LOG(ERROR, "Client onclose: handle=%d", m_handle);
        Close();
    }

    void FileMsgServerConn::OnTimer(uint64_t curr_tick) {
        if (curr_tick > m_last_recv_tick + SERVER_TIMEOUT) {
            RAW_LOG(ERROR, "Msg server timeout");
            Close();
        }
    }
    ...
    void FileMsgServerConn::HandlePdu(CImPdu *pdu) {
        switch (pdu->GetCommandId()) {
            ...
            default:
                RAW_LOG(ERROR, "No such cmd id = %u", pdu->GetCommandId());
                break;
        }
    }
    ...
    void FileMsgServerConn::_HandleMsgFileTransferReq(CImPdu *pdu) {
        ...
        do {
            ...
            if (task_id.empty()) {
                RAW_LOG(ERROR, "Create task id failed");
                break;
            }
            RAW_LOG(ERROR, "trams_mode=%d, task_id=%s, from_id=%d, to_id=%d, file_name=%s, file_size=%d",
                    transfer_req.trans_mode(), task_id.c_str(), from_id, to_id, transfer_req.file_name().c_str(),
            ...
            if (transfer_task == NULL) {
                ...
                // need_close = true;
                RAW_LOG(ERROR, "Create task failed");
                break;
            }
            ...
            RAW_LOG(ERROR, "Create task succeed, task id %s, task type %d, from user %d, to user %d", task_id.c_str(),
                    transfer_req.trans_mode(), from_id, to_id);
        } while (0);

        Network::SendMessageLite(this, SID_OTHER, CID_OTHER_FILE_TRANSFER_RSP, pdu->GetSeqNum(), &transfer_rsp);
    ...
    void FileMsgServerConn::_HandleGetServerAddressReq(CImPdu *pPdu) {
        ...
        for (std::list<IM::BaseDefine::IpAddr>::const_iterator it = addrs.begin(); it != addrs.end(); ++it) {
            ...
            *addr = *it;
            RAW_LOG(ERROR, "Upload file_client_conn addr info, ip=%s, port=%d", addr->ip().c_str(), addr->port());
        }

        Network::SendMessageLite((CImConn*)this, SID_OTHER, CID_OTHER_FILE_SERVER_IP_RSP, pPdu->GetSeqNum(), &msg);
    }
