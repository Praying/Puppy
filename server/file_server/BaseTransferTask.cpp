...
namespace Flow::FileServer {
    ...
    const char *GetCurrentOfflinePath() {
        ...
        if (g_current_save_path == NULL) {
            ...
            if (!getcwd(work_path, BUFSIZ)) {
                RAW_LOG(ERROR, "getcwd %s failed", work_path);
            } else {
                snprintf(s_tmp, BUFSIZ, "%s/offline_file", work_path);
            }

            RAW_LOG(ERROR, "save offline files to %s", s_tmp);

            int ret = mkdir(s_tmp, 0755);
            if ((ret != 0) && (errno != EEXIST)) {
                RAW_LOG(ERROR, "!!!Mkdir %s failed to save offline files", s_tmp);
            }
    ...
    static FILE *OpenByRead(const std::string &task_id, uint32_t user_id) {
        ...
        if (task_id.length() >= 2) {
            ...
            snprintf(save_path, BUFSIZ, "%s/%s/%s", GetCurrentOfflinePath(), task_id.substr(0, 2).c_str(),
                     task_id.c_str());
            fp = fopen(save_path, "rb");  // save fp
            if (!fp) {
                RAW_LOG(ERROR, "Open file %s for read failed", save_path);
            }
        }
        return fp;
    }
    ...
    static FILE *OpenByWrite(const std::string &task_id, uint32_t user_id) {
        ...
        if (task_id.length() >= 2) {
            ...
            if ((ret != 0) && (errno != EEXIST)) {
                RAW_LOG(ERROR, "Mkdir failed for path: %s", save_path);
            } else {
                ...
                if (!fp) {
                    RAW_LOG(ERROR, "Open file for write failed");
                    //break;
                }
            }
        }
...
//----------------------------------------------------------------------------
    BaseTransferTask::BaseTransferTask(const std::string &task_id, uint32_t from_user_id, uint32_t to_user_id,
...
//----------------------------------------------------------------------------
    ...
    bool OnlineTransferTask::ChangePullState(uint32_t user_id, int file_role) {
        ...
        do {
            ...
            if (!rv) {
                //
                RAW_LOG(ERROR, "Check error! user_id=%d, file_role=%d", user_id, file_role);
                break;
            }
            ...
            if (state_ != kTransferTaskStateReady && state_ != kTransferTaskStateWaitingSender &&
                ...
                RAW_LOG(ERROR,
                        "Invalid state, valid state is kTransferTaskStateReady or kTransferTaskStateWaitingSender or kTransferTaskStateWaitingReceiver, but state is %d",
                        state_);
                break;
            }
            ...
            if (state_ == kTransferTaskStateReady) {
                ...
                if (state_ == kTransferTaskStateWaitingReceiver) {
                    ...
                    if (file_role != CLIENT_REALTIME_RECVER) {
                        RAW_LOG(ERROR, "Invalid user, user_id = %d, but to_user_id_ = %d", user_id, to_user_id_);
                        break;
                    }
    ...
    int OnlineTransferTask::DoRecvData(uint32_t user_id, uint32_t offset, const char *data, uint32_t data_size) {
        ...
        do {
            // 检查是否发送者
            if (!CheckFromUserID(user_id)) {
                RAW_LOG(ERROR, "Check error! user_id=%d, from_user_id=%d, to_user_id", user_id, from_user_id_,
                        to_user_id_);
                break;
            }
            ...
            if (state_ != kTransferTaskStateWaitingTransfer && state_ != kTransferTaskStateTransfering) {
                RAW_LOG(ERROR,
                        "Check state_! user_id=%d, state=%d, but state need kTransferTaskStateWaitingTransfer or kTransferTaskStateTransfering",
                        user_id, state_);
                break;
            }
    ...
    int
    OnlineTransferTask::DoPullFileRequest(uint32_t user_id, uint32_t offset, uint32_t data_size, std::string *data) {
        int rv = -1;
        ...
        if (fp) {
            ...
            if (size == sizeof(file_header)) {
                ...
                if (file_size == file_header.get_file_size()) {
                    ...
                    RAW_LOG(ERROR, "Offile file size by task_id=%s, user_id=%u, header_file_size=%u, disk_file_size=%u",
                            task_id.c_str(), user_id, file_header.get_file_size(), file_size);
                }
            } else {
                RAW_LOG(ERROR, "Read file_header error by task_id=%s, user_id=%u", task_id.c_str(), user_id);
            }
            fclose(fp);
        }
...
//    }
        ...
        do {
            ...
            if (state_ != kTransferTaskStateReady &&
                ...
                RAW_LOG(ERROR,
                        "Invalid state, valid state is kTransferTaskStateReady or kTransferTaskStateUploadEnd, but state is %d",
                        state_);
                break;
            }
            ...
            if (state_ == kTransferTaskStateReady) {
                ...
                if (CLIENT_OFFLINE_UPLOAD == file_role) {
                    ...
                    RAW_LOG(ERROR, "Offline upload: file_role is CLIENT_OFFLINE_UPLOAD but file_role = %d", file_role);
                    break;
                    // state_ = kTransferTaskStateWaitingSender;
                }
            } else {
                if (file_role == CLIENT_OFFLINE_DOWNLOAD) {
                    ...
                    RAW_LOG(ERROR, "Offline upload: file_role is CLIENT_OFFLINE_DOWNLOAD but file_role = %d",
                            file_role);
                    break;
                }
            }
    ...
    int OfflineTransferTask::DoRecvData(uint32_t user_id, uint32_t offset, const char *data, uint32_t data_size) {
        ...
        do {
            // 检查是否发送者
            if (!CheckFromUserID(user_id)) {
                RAW_LOG(ERROR, "rsp user_id=%d, but sender_id is %d", user_id, from_user_id_);
                break;
            }

            // 检查状态
            if (state_ != kTransferTaskStateWaitingUpload && state_ != kTransferTaskStateUploading) {
                RAW_LOG(ERROR, "state=%d error, need kTransferTaskStateWaitingUpload or kTransferTaskStateUploading",
                        state_);
                break;
            }
            ...
            RAW_LOG(ERROR, "Ready recv data, offset=%d, data_size=%d, segment_size=%d", offset, data_size,
                    sengment_size_);
    ...
    int
    OfflineTransferTask::DoPullFileRequest(uint32_t user_id, uint32_t offset, uint32_t data_size, std::string *data) {
        int rv = -1;

        RAW_LOG(ERROR, "Recv pull file request: user_id=%d, offset=%d, data_size=%d", user_id, offset, data_size);
        ...
        do {
            ...
            if (state_ != kTransferTaskStateWaitingDownload && state_ != kTransferTaskStateDownloading) {
                RAW_LOG(ERROR,
                        "state=%d error, need kTransferTaskStateWaitingDownload or kTransferTaskStateDownloading",
                        state_);
                break;
            }
            ...
            if (state_ == kTransferTaskStateWaitingDownload) {
                ...
                if (sizeof(file_header) != size) {
                    ...
                    RAW_LOG(ERROR, "read file head failed.");
            ...
            // 检查offset是否有效
            if (offset != transfered_idx_ * SEGMENT_SIZE) {
                RAW_LOG(ERROR, "Recv offset error, offser=%d, transfered_offset=%d", offset,
            ...
            RAW_LOG(ERROR, "Ready send data, offset=%d, data_size=%d", offset, data_size);
            //if (data_size != GetNextSegmentBlockSize()) {
            //    RAW_LOG(ERROR,"Recv data_size error, data_size=%d, transfered_data_size=%d", data_size, GetNextSegmentBlockSize());
            //    break;
            //}
            ...
            if (NULL == tmpbuf) {
                // alloc mem failed
                RAW_LOG(ERROR, "alloc mem failed.");
                // SendPdu(&pdu);
                //t->unlock(__LINE__);
                // return;
                break;
            ...
            if (size != data_size) {
                RAW_LOG(ERROR, "Read size error, data_size=%d, but read_size=%d", data_size, size);
                delete[] tmpbuf;
                break;
                //
            }
...
/*
        ...
        if (0 != iret) {
            RAW_LOG(ERROR,"seek offset failed.");
            // SendPdu(&pdu);
            delete[] tmpbuf;
